...
注意：教程中提到的app.locals({key:value})方法经试验，报错，官网上express4.x不支持
  ...
  * 在响应中使用模板 
      ...
      locals是一个对象；
      回调函数在模板被呈现后执行，第一个参数是err,第二个参数是呈现后的模板的字符串形式。
      
      如果要把模板直接呈现为一个响应，可以使用res.render()，与app.render()的区别是它没有回调函数，app.render可以在发送之前，对数据做一些处理。
      
  * 示例参见:    express学习/在express中实现pug和ejs模板express_templates.js
  
  * ejs相关 参见 [ejs语法相关](./test/express学习/ejs语法相关.md)
  
  * pug/jade相关 参见 [pug学习](http://www.imooc.com/learn/259)
...
43. 学习笔记 见 [jade学习笔记]('../jade学习/jade学习整理.md')
